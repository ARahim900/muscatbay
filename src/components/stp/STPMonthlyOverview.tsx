...
export const STPMonthlyOverview: React.FC = () => {
  // Transform data for charts
  const chartData = stpMonthlyData.map(month => ({
    name: month.month.substring(5), // Just show MM format for cleaner display
    tankerVolume: month.expectedVolumeTankers,
    directSewage: month.directSewageMB,
    totalInfluent: month.totalInfluent,
    processed: month.totalWaterProcessed,
    irrigation: month.tseToIrrigation,
  }));

  // Calculate efficiency metrics for each month
  const efficiencyData = stpMonthlyData.map(month => {
    const processingEfficiency = (month.totalWaterProcessed / month.totalInfluent * 100).toFixed(1);
    const irrigationUtilization = (month.tseToIrrigation / month.totalWaterProcessed * 100).toFixed(1);
