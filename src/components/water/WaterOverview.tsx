...
import { ResponsiveContainer, AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, Legend, Pie, PieChart, Cell, Sector } from 'recharts';
...
const WaterOverview: React.FC<WaterOverviewProps> = ({
  ...
  const zoneDataForChart = useMemo(() => {
    return zoneMetrics.filter(zone => zone.bulkSupply > 0) // Filter out zones with zero supply
    .map(zone => ({
      name: zone.zone,
  ...
  const typeConsumptionData = useMemo(() => {
    return typeConsumption.filter(item => item.consumption > 0) // Filter out types with zero consumption
    .map(item => ({
      name: item.type,
  ...
  function getTypeColor(typeName: string, index: number): string {
    ...
    const colorArray = [waterColors.chart.blue, waterColors.chart.green, waterColors.chart.amber, waterColors.chart.red, waterColors.chart.purple, waterColors.chart.teal, waterColors.chart.lightBlue, waterColors.chart.indigo];
  ...
  return <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <EnhancedKpiCard title="Bulk Supply (L1)" value={levelMetrics.l1Supply.toLocaleString()} valueUnit="m³" icon={Droplet} variant="primary" description="Total water supplied to the system" className="transform transition-all duration-300 hover:shadow-lg" />
        
        <EnhancedKpiCard title="Zone Distribution (L2)" value={levelMetrics.l2Volume.toLocaleString()} valueUnit="m³" icon={BarChart2} variant="secondary" description="Water distributed to zones" className="transform transition-all duration-300 hover:shadow-lg" />
        
        <EnhancedKpiCard title="Total Consumption (L3)" value={levelMetrics.l3Volume.toLocaleString()} valueUnit="m³" icon={CircleDollarSign} variant="success" description="Total water consumed by end-users" className="transform transition-all duration-300 hover:shadow-lg" />
        
        <EnhancedKpiCard title="Total Water Loss" value={levelMetrics.totalLoss.toLocaleString()} valueUnit="m³" subValue={levelMetrics.totalLossPercentage.toFixed(1)} subValueUnit="% of bulk supply" icon={PercentCircle} variant={levelMetrics.totalLossPercentage > 20 ? "danger" : levelMetrics.totalLossPercentage > 15 ? "warning" : "neutral"} className="transform transition-all duration-300 hover:shadow-lg" />
      </div>
      ...
      <Card className="overflow-hidden bg-white dark:bg-gray-800 shadow-md border-0 rounded-xl">
        ...
        <CardContent className="p-6">
          {hasData ? <ResponsiveContainer width="100%" height={350}>
              <AreaChart data={monthlyTrendData} margin={{
            top: 10,
            right: 30,
            left: 20,
            bottom: 30
              ...
              stroke: '#cbd5e1'
                ...
                <Tooltip formatter={tooltipFormatter} contentStyle={{
              ...
              fontSize: '12px'
                ...
                <Area type="monotone" dataKey="Bulk Supply" stroke={waterColors.chart.blue} strokeWidth={3} fillOpacity={1} fill="url(#colorBulkSupply)" activeDot={{
              r: 6,
              strokeWidth: 1,
              stroke: '#fff'
            }} />
                <Area type="monotone" dataKey="Consumption" stroke={waterColors.chart.green} strokeWidth={3} fillOpacity={1} fill="url(#colorConsumption)" activeDot={{
              r: 6,
              strokeWidth: 1,
              stroke: '#fff'
            }} />
                <Area type="monotone" dataKey="Loss" stroke={waterColors.chart.red} strokeWidth={3} fillOpacity={1} fill="url(#colorLoss)" activeDot={{
              r: 6,
              strokeWidth: 1,
              stroke: '#fff'
            }} />
              </AreaChart>
            </ResponsiveContainer> : <div className="h-[350px] flex items-center justify-center bg-gray-50 rounded-lg">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card className="overflow-hidden bg-white dark:bg-gray-800 shadow-md border-0 rounded-xl">
          ...
          <CardContent className="p-6">
            {hasData && zoneDataForChart.length > 0 ? <EnhancedPieChart data={zoneDataForChart} innerRadius={0} outerRadius={130} className="h-[380px] w-full" showLegend valueFormatter={value => `${value.toLocaleString()} m³`} tooltipFormatter={(value, name) => <div className="text-sm">
                    <div className="font-medium">{name}</div>
                    <div>{value.toLocaleString()} m³</div>
                    <div className="text-xs text-gray-500">
                      {(value / levelMetrics.l1Supply * 100).toFixed(1)}% of total supply
                    </div>
                  </div>} /> : <div className="h-[380px] flex items-center justify-center bg-gray-50 rounded-lg">
        ...
        <Card className="overflow-hidden bg-white dark:bg-gray-800 shadow-md border-0 rounded-xl">
          ...
          <CardContent className="p-6">
            {hasData && typeConsumptionData.length > 0 ? <EnhancedPieChart data={typeConsumptionData} innerRadius={0} outerRadius={130} className="h-[380px] w-full" showLegend valueFormatter={value => `${value.toLocaleString()} m³`} tooltipFormatter={(value, name) => <div className="text-sm">
                    <div className="font-medium">{name}</div>
                    <div>{value.toLocaleString()} m³</div>
                    <div className="text-xs text-gray-500">
