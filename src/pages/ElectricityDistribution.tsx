...
const ElectricityDistribution = () => {
  ...
  const [consumptionByCategory, setConsumptionByCategory] = useState([]);
  const [consumptionByZone, setConsumptionByZone] = useState([]);
  ...
  const processData = (data) => {
    ...
    const total = data.reduce((sum, item) => {
      const itemTotal = Object.values(item.consumption).reduce((a, b) => a + b, 0);
    ...
    const max = data.reduce((currentMax, item) => {
      const itemMax = Math.max(...Object.values(item.consumption));
    ...
    const byCategory = uniqueCategories.map(category => {
      ...
      const categoryTotal = categoryItems.reduce((sum, item) => {
        return sum + Object.values(item.consumption).reduce((a, b) => a + b, 0);
      }, 0);
