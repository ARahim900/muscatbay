...
| Tests / checks | Vitest (326 tests), ESLint (incl. the design rule on migrated folders), `tsc --noEmit`, `next build` (+ `mobile/` `tsc --noEmit`) — all green as of 2026-09-03 |
...
- **Water Daily — the month-to-date cumulative balance chart counts a missing
  L2 reading as 0 supply** (`buildZoneMtd` in
  `components/water/daily-report/daily-metrics.ts`: `cs += p.l2 ?? 0` while the
  day's ΣL3 is still added). On such a day the cumulative supply is understated
  and the cumulative loss line dips, which is the "missing ≠ zero" rule broken;
  the card's footer discloses it but the arithmetic is still wrong. The chart is
  the running sum of the "Zone daily consumption trend" chart directly above it
  (same three series, integrated from day 1), so its only distinct signal is a
  change of slope in the loss line. Owner asked on 2026-09-03 whether it is
  redundant; recommendation given: remove it and carry the month-to-date loss
  figure into the daily chart's footer, or keep it and skip days with no L2
  reading from both sums. Awaiting his decision — no change made.
...
- **#79 Design System v2.0 — foundation + Water (2026-09-02).** Branch
  ...
  owner's request (see the Water — Daily row and §4) and awaits his decision.
