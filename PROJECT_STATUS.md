...
| Water — 3D Map | `/water` (3D Map tab) | ✅ Live (added 2026-07-19) | `water_meters` + `water_monthly_consumption` (monthly) · `water_daily_consumption` (daily) | Third Water subsection: an interactive **Three.js** 3D operational map of the zones (3A, 3B, 05, 08, Village Square + FM). Reuses the existing calc engines (`computePeriod` monthly, `buildZoneWatch` daily) — no new thresholds. Zone platforms/pillars coloured by loss severity, instanced meter nodes, trunk links + pausable flow particles, click-to-inspect zone/meter panels, search + filters, monthly/daily modes with date steppers, and an **accessible table fallback** (auto-primary when WebGL is unavailable). Lazy-loaded (`ssr:false`) so Monthly/Daily stay fast. **Free/offline** — no Google/MapLibre/tiles, no API key. **Real aerial imagery (2026-07-19):** three calibrated local image scenes (`core` 3A·3B·05·VS · `east` Z08 & Hotel · `fm` Zone FM) rendered as textured planes with programmatic severity pins at verified bulk-meter coordinates (+ Hotel JMB & Z08 IRR meters); calibration contract + least-squares alignment fit in `lib/water-map-scenes.ts` (unit-tested); clean base layers in `public/water-map/` (blue calibration ink patched out; marked sources kept in repo-root `Zones Pics/`); zone selection loads its scene and flies the camera. Un-surveyed meters are deliberately NOT drawn (no illustrative placement) — they surface via search/filters/panels/table; `properties` polygon extension point ready for manually-traced building footprints. All consumption/loss/efficiency values are live. See `WATER_3D_MAP.md` |
...
- **Water 3D Map (third subsection) — new (2026-07-19).** Adds an interactive
  Three.js 3D map to `/water` alongside Monthly/Daily, wired to the live water
  tables via a pure adapter over the existing calc engines. Free/offline, no API
  key; zone positions are surveyed (real coordinates provided 2026-07-19),
  per-meter positions provisional. type-check / lint / build / tests green. See
  `WATER_3D_MAP.md`. Awaiting review/merge.
