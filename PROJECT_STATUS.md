...
**Last curated review:** 2026-09-20
...
| Water — Monthly | `/water` (Monthly tab) | ✅ Live, fully dynamic | `water_meters` + `water_monthly_consumption` | Balance A1/A2/A3, zones, buildings, DC, exceptions; months appear automatically (see §3). **2026-09-02 — first page on Design System v2.0** (`/DESIGN_SYSTEM.md`; branch `claude/supabase-service-role-setup-zgjzno`, PR #79 — kept as a draft through six review rounds on the Vercel preview; on 2026-09-03 the owner delegated the merge decision, the PR was taken out of draft for the automated review and merged after it). The page now reads, top to bottom, `Breadcrumb` → `PageHeader` + one `StatusChip` → `SegmentedControl` (Monthly / Daily / Satellite) → six `KpiCard`s (always visible — no longer gated behind the *Overview* sub-tab) → the kit `DateRangePicker` (ONE period control, replacing the year buttons, slider, start/end selects and 3M/6M/1Y/YTD row; because the balance is computed per calendar year, a cross-year pick snaps its start to the first data month of the end month's year) → five `Tabs` (Exceptions carries its count) → `SectionCard`s with `ChartFrame`/`chartTheme` charts, purple `text-eyebrow` table headers and `Badge` severity chips. The local `--wm-*` palette, `Panel`, `Kpi`, `TabNavigation` and `PeriodFilter` are gone; zone/building severity uses the §2.2 status tokens with the band label always beside the colour; data, copy and calculations are unchanged. (The earlier 2026-09-02 shared-pattern work — `ChartShell`, a Water `OperationalBriefing` — was reverted the same day at the owner's request and is not in the tree.) **Owner review of the preview (same day):** Water must match HVAC and STP, not the other way round — the KPI row is back on the app-wide `StatsGrid` tile (colours by meaning: A1 `water`, A2 `info`, A3 `success`, Efficiency `success`, Total Loss `danger`, Loss Cost `warning`), the mode switch and section tabs are rendered by the app-wide `TabNavigation` strip (purple pill / STP card pill), the period control's presets and selects are visibly pressable with the active preset filled, the donut is the HVAC donut (labelled slices, `CHART_PALETTE`, legend below), and the blue month-to-date callout is gone (its provenance is one clause in the caption under the period control). Recorded as rulings in `DESIGN_SYSTEM.md` §7. **2026-07-06:** Zone Analysis now surfaces the **primary/trunk network (A1→A2)** as a first-class item — the loss *above* every zone that the per-zone drill-downs structurally can't show. It has a top-of-section banner, its own dropdown option + drill-down (KPIs, main-bulk-vs-reached bar, monthly A1-vs-A2 trend, and an A1 reconciliation table of Σ zone-bulk + Σ direct vs main bulk), plus an Exceptions-register row when the A1→A2 gap breaches target or goes negative (A2>A1 → main-meter/timing issue). Live A1→A2 gap has run 0–41% month-to-month (even negative in Jan/Feb-26), so it is material and reading-sensitive. The long tables (A1 reconciliation, individual meters, exceptions register, main database) now carry a **10/20/50/All rows selector** so operators cap what shows instead of scrolling a fixed box (drill-down tables default to 20; the database defaults to All) |
| Water — Daily | `/water` (Daily tab) | ✅ Live, rebuilt 2026-07-04 | `water_daily_consumption`, `water_loss_daily/summary` | Zone-first leak-detection dashboard, 5 sections mirroring Monthly: Zone Watch (static briefing card — the looping ticker was retired 2026-09-02 under DESIGN_SYSTEM.md rule 7 — zone performance table, zone×day loss heatmap), Zone Analysis (drill-down: gauges, day composition + daily trend side by side, L3 meter table), Direct Connections, Daily Database (meter×day ledger, flags, CSV), Exceptions (auto register: high loss, negative balance, missing L2, rising-loss signature, spikes, zero-streaks). Daily balances were distribution-level only (L2 vs ΣL3) until Apr-26 — since then the NAMA main bulk lands daily in `water_daily_consumption` under account `C43659` ("Main Bulk (NAMA)", zone `Main_Bulk`); fed by CSV upload / Grafana sync. **2026-07-10:** Zone Analysis L3 meters table now shows the meter **name** (from `meter_name`, e.g. "Building FM", "Irrigation Tank (Z01_FM)") in the Meter column, with the account number kept in the Account column — previously both columns repeated the account number. **2026-07-23:** Direct Connections gauges upgraded from the two-ring L2+DC vs L3+DC pair to a **three-stage supply chain — Main Bulk (C43659) → zone bulks + DC → individual meters + DC** — with the loss written between each pair, mirroring the monthly A1→A2→A3 balance (`MAIN_BULK_ACCOUNT` in `lib/water-accounts.ts`); days/months without a main-bulk reading (pre-Apr-26) show an explicit "no reading" note and fall back to the distribution-level pair instead of faking a zero supply. The DC trend chart below the gauges now plots the same series day-by-day — Main Bulk line vs a zone-bulks-+-DC line (gap between them = daily trunk loss), with the DC-only share kept as a soft area; missing main-bulk days gap the line rather than plot zero. **2026-09-02 — Design System v2.0:** every Daily panel is a `SectionCard`, KPI tiles are `KpiCard`, status pills are `Badge`, charts go through `ChartFrame`; the Daily view reports its fetch/realtime state up to the page's single `StatusChip` instead of rendering its own Live/Offline pill; the Zone Analysis zone picker is a select (the same control Monthly uses). **2026-09-03 (QA round):** the KPI rows render the app-wide `StatsGrid` tile (the `KpiCard` adapter now wraps it), the five section panels carry `panel-<key>` ids so the `Tabs` strip's `aria-controls` resolves, and the Daily view stays reachable when the monthly read fails — the page's mode switch no longer disappears with the monthly error state. **Zone Analysis charts, reviewed 2026-09-03 at the owner's request:** "Zone daily consumption trend" plots each day's L2 bulk, ΣL3 and loss; "Month-to-date cumulative balance" plots the running sums of the same three series from day 1, so its lines are straight when the daily rates are steady and its end points are the month's totals (Zone FM Aug-26: 439.71 m³ cumulative loss = 41.1 % of supply, consistent with the ~14 m³/day the daily chart shows). It was working; it was also derivable from the chart above it, and it counted a missing L2 day as 0 supply. **Owner's decision, same day: the cumulative chart is removed** (`zone-mtd-chart.tsx` and `buildZoneMtd` deleted). The daily trend now sits at half width on the right with a footer carrying the month-to-date balance (only days with a bulk reading enter both sums, so a missing L2 day is skipped, not counted as 0), and the left half is a new chart, **"Where the water went — Day N"** (`components/water/daily-report/zone-day-breakdown-chart.tsx`, data from `buildZoneDayBreakdown`): the day's L2 bulk split into the zone's six largest L3 meters, the rest folded into one "Other (n meters)" bar, and the unmetered loss ranked alongside them in the loss colour — so when the loss outranks every meter (Zone FM, 31 Aug-26: 15.48 m³ unmetered against 4.92 m³ for the largest building) that is the first bar. An unread bulk meter means no loss bar and a footer saying so; unread L3 meters are counted in the footer, never treated as 0. The trend answers "when", the new chart answers "who". **2026-09-10 — Direct Connections made auditable.** The middle gauge is Σ zone bulks (L2) + Σ direct connections (DC), but nothing on the tab listed the zone bulks, so the figure could not be checked against the table below it. `DCDailyTable` (DC meters only) is replaced by **`SupplyReconciliationTable`**: the NAMA main bulk, all seven zone bulks (ZEN Project / Zone_03C included) and every DC meter, day by day, with a ΣL2 line, a ΣDC line, the combined **ΣL2 + ΣDC** line that *is* the middle gauge, and a trunk-main loss line (L1 − combined). Arithmetic lives in the pure, unit-tested `components/water/daily-report/supply-reconciliation.ts` (`buildSupplyMatrix` / `supplyDaySnapshot`), which now also feeds the gauges and the trend chart so the three cannot drift apart. The gauges carry a read-coverage line ("built from 7 of 7 zone bulks and 9 of 9 DC") because an unread bulk sums as 0 and would otherwise understate the total silently. **Same change — TSE removed from the DC balance:** `DC_METERS` listed the two TSE irrigation controllers (`4300340`, `4300341`), which run on treated effluent and are `label = N/A` in the registry, so treated-effluent volume was sitting on the potable side of the NAMA balance (Jun-26: 155.70 m³ overstated in L2 + DC, trunk loss understated by the same). They moved to a new `TSE_IRRIGATION_METERS` list in `lib/water-accounts.ts` alongside `4300322` / `4300347`; `DC_METERS` is now exactly the nine `Direct_Connection` meters in the registry, matching the Monthly module. **2026-09-10 (owner review of the same tab).** The trend chart moved to the app's line-chart idiom — two gradient-filled `Area` series (Main Bulk purple, Zone Bulks + DC teal, 0.28 → 0.04 stops, the same treatment the STP volume chart uses) with the **trunk loss as red `chart-loss` bars on their own right-hand axis**, so the gap between the two lines is also readable as a bar height instead of being inferred. A negative gap draws no bar — the network measuring more than the main bulk is a reading-basis mismatch, not water lost. The DC-only area was dropped; the DC share is in the table. In the reconciliation table each of the three sections (main bulk, zone bulks, direct connections) is independently collapsible — subtotals and the combined line stay on screen whatever is folded — and trunk-loss cells carrying an actual loss are tinted `bg-danger-tint` so costly days are findable across a 31-column matrix **2026-09-15 — meter coverage on the gauges (owner request):** ΣL3 counts an unread L3 meter as 0, so a day where 23 of 31 meters reported showed a smaller ΣL3 and a larger apparent loss with nothing on the gauge to say so — and the owner's rule is to act on a gap only when the full set reported (a partial day is a meter or feed to repair, not a leak). The ΣL3 gauge's sublabel is now **"23 / 31 meters reported"** — and that is all the gauge says: a first cut also put an amber note beneath the gauges listing every unread meter by name, and the owner removed it the same day ("no need to show that box") in favour of the L3 table below, where **every day cell with no reading is tinted light red** (`bg-danger-tint`, a red dash with a "No reading" label; a recorded 0 is a reading and stays plain) — so the meter that went quiet, and the day it went quiet, are found by scanning the matrix; the loss connector's caption appends "· n unread" so the m³ figure is read as an upper bound; the Zone Watch table shows **reported / configured** under each zone name (warning colour + icon when short) and exports a "Meters reported" column; and the Exceptions register gains a **"Meters not reporting"** Watch row per zone naming up to six missing meters. All from one pure function, `zoneMeterCoverage` in `daily-metrics.ts` (`l3Reported` added to the day series and the fleet rows). Live September facts that prompted it: **4300337 Cabinet FM (Zone FM) and 4300187 D‑62 Building Bulk (Zone 3B) have no September row at all**, so those two zones can never show a full set as configured; **4300180 D‑46 Building Bulk (Zone 3A) stopped on 8 Sept**; and 13 Sept was partial across Zone 3A (23/31), 3B (25/34) and Zone 8 (17/22), with whole groups (building bulks, villas Z8‑18…22) missing together — a feed lag, not seven failures. The owner has ruled that **no meter is removed from the configured lists**: the gap is to be shown, then repaired |
| Water — Satellite View | `/water` (Satellite View tab) | ✅ Live, added 2026-07-30 | same fetch as Monthly + `water_daily_consumption` (L1 series) | As-built network map over satellite imagery: 13.7 km of extracted COO87 pipework, 119 plot-joined meters, zone loss gauges, daily timeline. The map engine is the field-verified standalone viewer, self-hosted at `public/satellite/` and embedded in a same-origin iframe; `components/water/satellite/satellite-model.ts` projects the SAME `waterMeters` array the Monthly tab renders into the engine's payload (posted via `satviz:data` handshake), so the two views cannot disagree — no figures are baked into the engine. Geometry (pipes/plots/tank) is static as-built survey data and deliberately NOT in Supabase. `proxy.ts` grants `/satellite/*` its own CSP (`frame-ancestors 'self'`, tile hosts, blob workers) — everywhere else keeps `frame-ancestors 'none'`. Latest month renders as month-to-date, labelled. **2026-07-30 (2):** right detail panel is collapsible (collapsed by default, auto-expands when a zone/meter is selected, header mirrors the panel title); zone selection and map-cards are dropdowns; "Zones only" + a selected zone shows only that zone's card; months with no bulk reading show — instead of a computed 0%. **2026-07-30 (3):** Data view selector — Monthly (with a Month dropdown: period total or any single month; every card, tree row, plate, gauge and the topbar move together, nulls stay —) vs Daily (month dropdown + date slider driving the timeline; L1 only, capped at the last day read); camera buttons consolidated into a labelled Camera dropdown + toggle group; zone cards anchored at the zone's supply-side entry (nearest surveyed plot to the NAMA point) instead of the villa-cluster centre. Fixed: `getDailyWaterConsumptionFromSupabase` now paginates past PostgREST's 1,000-row cap — the unfiltered call was silently dropping July's daily series (2,443 rows in table). **2026-07-30 (4):** villa cards no longer fan out into a floating cloud — each card sits directly over its villa or is culled for that zoom (largest consumer first; zooming in reveals the rest, and a culled villa is one plate-hover away); "All" mode zooms the selected zone in closer by default and re-frames on card-mode change; the selected zone's map card collapses to its pill whenever villa cards are on show; collapsing the right panel now re-runs the card layout so cards under it come back. **2026-07-30 (5, owner request):** zone focus dimming — selecting a zone drops every other zone's ground plates, plot fills and floating cards to low opacity (hover restores a dimmed card), so the chosen zone owns the eye; and the expanded zone map-card now carries the full story instead of stopping at the unbilled %: a bulk / individual / loss m³ trio (mirroring the right panel's gauge) plus the account count (L3 + L4 apartments) in the foot. **2026-07-31 — Stage 1 of the owner-approved redesign (foundation only):** design tokens in the engine (`--s-*` 8px spacing scale, `--r-panel:16px`/`--r-card:12px`/`--r-ctl:8px`, `--ctl-h:44px`, three-level type scale), ONE panel style (96% solid, 16px radius — `#viewctl` and `#hint` now carry `.panel`), ONE uppercase label style (shared selector list; per-rule variants deleted), ONE selected/hover state (`--sel-bg`/`--hov-bg`), serif display face (Roboto Slab) fully retired — Inter everywhere, panels on 16px offsets and 280/320px widths, all controls 44px, chips 32px. Zero logic changes: verified in headless Chromium (boot, zone select + focus dimming, card trio, daily slider/timeline — all intact, zero JS errors). **Stage 2 (2026-07-31, awaiting owner review before merge):** ONE visible period selector — the left panel's Month control; the zone panel's period-chip row and the timeline's month chips are removed (`zonePeriod` and `monthIdx` follow the left selectors, all calculations unchanged); Camera + Map cards moved into one collapsed "View & camera" popover bottom-left (all controls keep their IDs/bindings — pure re-parenting + a show/hide toggle); the bottom bar is Daily-only now (`#tl[hidden]{display:none}` — the author `display:flex` had been defeating the `hidden` attribute); default map-card density is Zones-only (Top 10/All are one popover click away). **Stage 3 (2026-07-31, awaiting owner review before merge):** overlay density — the selected zone's map card auto-expands ONLY while the right panel is collapsed (open panel already tells the story; panel toggle re-renders cards; the caret still overrides); right-panel hierarchy reordered to title → period → loss gauge → bulk/individual/loss trio → explainer → month table → trend → guidance, with the bulk-meter account + position tag demoted to tertiary meta at the bottom; responsive — ≤860px the left panel folds behind a "Filters" button instead of the old `display:none` (which silently removed search/period/zone from tablets), ≤640px the right panel becomes a full-width bottom sheet (60% max-height). Verified at 1600/800/600px: zero page errors, list-driven zone selection works on tablet, all calculations untouched. **2026-08-04 (2):** network.js re-extracted with DWG CURVES TESSELLATED — the first extraction read polyline vertices only, so every bulged road bend collapsed to its chord and the main sliced over buildings (reported on Zone 3B; a 192 m chord is now a 49-vertex curve). `dwg_extract.py` (Water Network Viz `_build/`) flattens bulges/arcs/splines via ezdxf paths at 15 cm sagitta; 423 features, 1,238 base vertices, 13.81 km; needs LibreDWG ≥ 0.14 from git (brew 0.13.3 cannot decode the sheets). Reconciled with the 04-Aug owner-marked road corrections: each mark measured against the curve-true geometry — both Zone 5 marks retired (1.0 m / 4.5 m from the DWG curve, same corridor), Zone 3 (23.6 m) and both Zone 8 corridors (~11 m) KEPT as owner field knowledge beyond the drawing. `network-road-corrections.js` now matches runs by bore/material/junction endpoints (never vertex count) and splices marks between nearest surviving vertices, so re-extractions can't silently no-op it. **2026-08-04 (3, QC audit):** the Zone 3 owner mark itself was the remaining defect — a footprint-intersection audit (96 mains × 271 building polygons, 0.25 m sampling) showed the digitised mark ran 39.5 m + 32.3 m THROUGH two Zone 3B buildings the surveyed curve clears by 4–10 m (independently confirmed against the master as-built DXF: mark crosses wall linework 158×, curve 0×). Mark RETIRED — Zone 3 rides the 49-vertex surveyed curve; only the two Zone 8 corridors keep owner splices (they cross nothing). Final audit: ZERO mains-through-buildings ≥ 1.5 m site-wide (worst residual contact 1.25 m, below tolerance). Effective network 1,252 vertices, `roadAlignedFeatures` 2; corrections cache-buster bumped to v=3 (deployed v=2 still carried the bad splice). Latent note: four 2-vertex stub twins sit within the 1.5 m matcher tolerance — documented in the file, no live target touches them. **2026-08-04 (4, owner request):** building-footprint overlay — the 251 as-built building outlines (true rings where drawn, oriented rectangles labelled indicative elsewhere) now render as a quiet white outline layer beneath the network (`data/building-footprints.js`, UTM→WGS84 via the same EPSG transform as the pipes; rebuild via `_build/footprints_to_wgs.py` in the Water Network Viz folder). Zoom-gated (fades in ≥ z15), "Buildings" toggle in View & camera, legend entry; drawn first so pipes/plates/cards always stack above. Verified: independent pyproj recompute max deviation 0.074 m (rounding only), 279 tests incl. 3 new data-integrity tests, adversarial integration review clean. **2026-08-04 (5, owner request — Village Square):** sub-connections to villas/buildings now render in their own colour in every zone — gold #E8C064 core over a dark casing (hydrant/washout/valve legs moved to soft red #D67A7A matching hydrant points; teal stays reserved for Zone FM + approved as-built overlays), legend updated. Village Square zone panel now carries the building-connection schedule transcribed from the OSCO as-built set AB-MB-VS-0173-PL-WS-001/002 + WSS-003 Rev C (MWMC-01 110Ø main meter chamber, per-shop meters with locations, VCWM-01…04, VC-01…08, 110Ø DCWS HDPE roof-tank filling line) — listed, not drawn: a full georeferencing attempt (DWGs converted via LibreDWG-git, PW-01/02/03 grid-georeferenced with passing scale checks, curve/context ICP fits) established the OSCO sheets carry NO geographic control (local Revit frames, unbound architecture xrefs, raster-only plots) and the 2018 COO87 survey predates the crescent, so no shared geometry exists to fit against — publishing a guessed transform would violate the never-fabricate rule. The surveyed 225 HDPE trunk + FM Road 3/2A junction fronting the crescent are already drawn from COO87 (trunk-to-cabinet tapping not surveyed); overview + zone notes updated with the verified state and the unlock path (field-GPS MWMC-01 cover, VC-01, one hose bib → similarity fit with reported residuals). Full search documented in `public/satellite/INTEGRATION.md`. **2026-09-16 — iOS compatibility:** when Safari cannot initialise MapLibre/WebGL, the iframe now switches to a non-WebGL satellite compatibility map using the same daily payload, mapped meter positions and network geometry. The fallback keeps pan, zoom and tappable meter details, preserves missing readings as —, and reports a degraded rather than failed state to the host. **2026-09-16 follow-up:** the single ArcGIS export image failed on real iOS Safari, leaving only the network overlay; compatibility imagery now uses independently loaded, same-origin proxied Esri XYZ tiles so one failed tile cannot blank the map; the iframe and renderer URLs are explicitly versioned, and cache v13 refreshes installed PWA clients without one stale render. The normal MapLibre path remains unchanged for capable browsers. **2026-09-19:** the daily map (`consumption.html` / `consumption-engine.js`) now draws flat house outlines from `data/villa-buildings.js` (319 outlines from the as-built wall lines, gardens removed using the 11-Jan-2024 WorldView-2 image; 193 built drawn solid, 126 plots with no building on that image or not yet traced drawn dashed; zoom ≥ 15, dimmed with the zone like the network). Selecting a villa meter highlights its outline and its house connection with a note giving size, material and length and stating the match is by position, to be confirmed against the meter schedule (117 villas; Zone 8 building matches are approximate; Z8-5 and Z8-10 have no building record). A **3D** map button (top right, off by default) tilts the map to 55° and raises the 193 built houses (MapLibre `fill-extrusion`; the selected villa in teal) with an on-screen "building heights assumed" note — the drawings carry no heights. The map still opens flat, and touch tilt and rotation stay off, so the September iOS behaviour is unchanged. Cache bumped to v16 (iframe, scripts, service worker). **2026-09-19 (2, owner request):** automatic zone camera, as in the standalone as-built 3D model — selecting a zone frames the WHOLE zone in one move at an oblique angle (pitch 55°, bearing 28°, camera south-west looking north-east) with that zone's houses raised; neighbouring zones stay as dimmed outlines; after the move the engine checks every zone point is on screen and steps back if not. The whole-site view stays flat. The 2D/3D button is the operator's override and is respected for the session. Below 640 px the map stays flat unless 3D is pressed. Map-side controls (3D button, heights note, house-connection note) now take the app's synced tokens, so they follow light and dark. Cache v17. **2026-09-20 (owner feedback: hard to move on a phone, controls less direct than the standalone model):** (1) **Full-screen map** — a Full screen button on the map card, and on touch screens the first tap on the embedded map opens it full screen (`SatelliteMap.tsx`, same iframe, no reload; Escape or the button closes it). Embedded, a touch map still needs two fingers so the page can scroll; **full screen, one finger moves the map**, two fingers turn and tilt it, and the compass resets north (`satviz:mode` → `applyMode()` toggles MapLibre cooperative gestures, rotation and touch pitch). (2) **Zone chips on the map** — All + every zone that has coordinates, one tap each, the selected one filled purple and scrolled into view; the host now accepts `satviz:select-zone` for any zone in its list, and `""` for All. (3) The automatic angled zone view now applies on phones too (the 640 px cut-off is gone; the 2D button remains the override). Cache v18. **2026-09-20 (2, owner rulings on the map labels):** figures are whole numbers (10 m³, not 10.00 m³; below 10 one decimal is kept and a non-zero reading under 0.05 shows "<0.1", because a villa's daily use is often under 1 m³ and must never read as 0; no reading stays —); label boxes are near-black `#0A090C` for contrast over imagery; the **zone bulk meter** is always sent to the map with the selected zone (`mapMeters` in `satellite-view.tsx`, tables and totals still follow the level filter) and drawn apart — white card with a "Bulk meter" tag, larger figure, white dot with a purple ring, never hidden, and excluded from the scale that sizes the individual meters' dots; crowded labels shrink to the figure alone before being hidden (every meter keeps its dot; labels are placed highest consumption first, and zooming in makes room for the rest). Cache v19. **2026-09-20 (3, owner's eight-point review of the preview):** (1) the page **opens on the latest recorded day** (this month, else the month before) instead of yesterday, which is often not entered yet and made the map read as broken; a link that names a date keeps it, and a link without one keeps following the latest day. (2) **Reading day** is the Daily report's control — date field for the month, chevrons, a day slider capped at today, and a "Latest recorded" button; day steps replace the URL entry rather than flooding Back. (3) **Meter status** — `meterStatus()` in `consumptionModel.ts` classes every reading as normal / high usage / zero reading / no reading. *High* reuses the Daily report's `detectSpike` (≥ 2× the trailing 7-day average and ≥ 5 m³ above it, ≥ 3 prior readings), so the map and the report cannot disagree; the hook now always reads the previous month too (baseline and the month-start default). A **Status** filter (with counts) narrows the map and the table; totals stay on the zone and level. Dots are coloured by status and every label says it in words; a zero is drawn as large as a missing reading; a crowded finding keeps its meter name (no more bare "— m³"). (4) **Zone loss on the overview** — `summariseZoneLosses()` writes each zone's bulk − L3 on its marker using the Daily report's `dailySeverity` scale; when the bulk reading or any L3 reading is missing it says so and shows —, never an estimate. (5) **Zone markers never overlap**: each is a point with a card that may hang off any of eight sides, full, without the reporting line, or name-and-figure only; a small backtracking search picks the arrangement. (6) **The map is flat unless the operator presses 3D** — selecting a zone frames it but no longer tilts it (this reverses ruling 2026-09-19 (2)). (7) Fixed: tapping a label could open a different meter — labels sit inside the map, so the tap also reached the dot lying under the label and that one answered last; the dot layer now ignores taps that did not land on the map itself. (8) Overlays: the summary box that covered the first zone chip is gone — the day's total and reporting count sit in the card header (and in a one-line strip in full screen); the zone chips are the app's pill strip in one scrolling row; meter details are a **bottom sheet on a phone** and now follow the map into full screen; the house-connection note is written in that sheet and drawn on the map only at ≥ 640 px. (9) **Unmapped meters** list under the map, grouped by zone with counts — positions are never guessed. (10) Legend cut to three items with the rest behind "About this map". Cache v20. **Data finding (live table, 2026-09-20):** Sep-26 has readings to 17 Sep (422 of 429 meters); 18–20 Sep are empty. On 17 Sep the Main Bulk C43659, Zone 8 bulk 4300342 and Zone 3B bulk 4300344 are still NULL — those three rows were last updated together on 17 Sep 03:31 UTC, a day before the other bulks (18 Sep 20:48 UTC), which is consistent with the Kalhat hand-reading path running a day behind (to be confirmed with the person entering them). Their loss shows — until 17 Sep is entered; no code can or should fill it. **2026-09-20 (4, owner-approved rebuild of the section as one frame — studied from utility command-centre dashboards, drawn in the app's own theme):** the Satellite tab is now **panels · map · panels with a KPI strip beneath**, the same frame at three levels (`SegmentedControl`: All zones → Zone → Meter). **The map carries no figures**: zones are named pins, meters are status dots, and a name tag appears only for the selected meter, the zone bulk, or (with a mouse) the dot under the pointer — the black label boxes, the white house-connection note and the purple details panel are gone. Left: `ZonesPanel` (zones ranked by unaccounted water; a row selects the zone), `StatusPanel` (counts; a row filters map and list), `TrendPanel` (7 days, bulk in against metered L3, gaps never joined). Right: `MetersPanel` (findings first, search and meter-level select; a row and its dot select each other) and `SelectedMeterPanel` (status, note, house connection, `MeterDetails`). `SatelliteSummary` is four `StatsGrid` tiles for the level on screen. On a phone the same blocks stack under the map; in full screen the zone strip and a slim meter sheet return because the panels are out of view. **Auto fly:** only a change of zone moves the whole camera — it stands behind the zone's start (its bulk meter; where that is not mapped, the end of the zone's long axis nearer the site) and looks to the far end at 35°, whole zone in frame (`flyBearing`, `keepInView`); a meter tapped on the map never moves the camera, one chosen from the list is centred at the same angle; an **All** map button returns to the whole site; 3D (55°, raised houses) is still only the button. **Partial loss (owner ruling):** `summariseZoneBalance` now returns the measured bulk − L3 difference when some L3 meters have not reported, flagged `partial` — as the Daily report does — instead of withholding it; a zone with one silent meter (Zone 3A's D-46, silent since 8 Sep) otherwise never showed a figure. No reading is ever filled in. Cache v21. **2026-09-20 (5, owner ruling on the markers — "circles and colouring look unofficial; use a percentage ring, green good / amber medium / red bad"):** every mapped meter is now **one ring of the same size** (`buildRing` in the engine, `RingGauge` in the panels — one shape on the map, in the lists, in the legend and beside the selected meter). The ring **fills with the day's reading as a share of that meter's own average over its last recorded days** (up to 7, at least 3, missing days skipped not counted as zero), full at twice it, and takes the colour of its band: **normal below 130% (green), elevated 130–200% (amber), high usage 200% and above (red)**. A zero reading is an empty ring marked 0; no reading, or too few recorded days to have an average, is a dashed grey ring with no verdict. Size no longer varies with m³ — the bubble-chart look is gone, and the selected meter is ringed in purple. `meterStatus()` was rewritten around this ratio and returns `baseline` and `ratio` with the status; the old dependence on the Daily report's `detectSpike` is removed **because its 5 m³ floor hid villa-size jumps** — on 17 Sep it missed D-44 (2.3 → 6.1 m³, 261% of usual) and Z3-33 (0.06 → 3.6 m³). `MeterStatus` gains `elevated`, so the Status panel and filter now carry five bands. The map's circle and hit layers are gone: markers are DOM rings, clicked directly, and with a mouse a ring answers with its name, figure and percentage. Cache v22. **2026-09-20 (6, owner ruling — the ring gauges read as "traffic and invisible, unclear"):** the marker is now a **segmented bar**, on the pattern the owner supplied: five segments on a near-black track (`#0A090C`, the same contrast device as the earlier label boxes), lit **one per 40% of that meter's own usual and all five from twice it**, in the saturated `--status-*` colour of its band — green normal, amber elevated, red high. A zero reading lights none and carries an amber edge; no reading, or too few recorded days for an average, is a dashed bar dimmed to 60% so an absence can never be read as a recorded zero. The bands, the ratio and everything else from (5) are unchanged — only the shape. `StatusBar` (panels, lists, legend) and `buildBar` (both map renderers) are the one implementation; the ring is gone. The bars scale with the zoom (0.62 to 1.15 between z15.4 and z17.2), so a dense zone does not crowd at zone scale. Cache v23. **Fixed in the same round, found on the preview:** the opening day followed *any* reading, so one meter entered early for 19 Sep pinned the page to a day where nothing else was recorded — the very fault the default was added to fix. `latestRecordedDay()` now takes a minimum share of reporting meters; the opening day asks for half, while the "Latest recorded" button still reports any reading for the meters in view. Not yet tested on a real iPhone or iPad — test the Vercel preview on the device before merging. Not yet checked in the logged-in app with live data. **2026-09-20 (7, owner ruling — the segmented bar is "too big"; "this indicator should be a small line to keep only visible"):** a meter is now **one small line**, 4px tall, in the colour of its band and the longer for how serious that band is — 10px normal, 14px elevated, 18px high usage; a zero reading and a meter that never reported are 5px dots, amber and grey, the missing one dimmed. The near-black track is gone: over 30 villas the bars read as a wall of boxes and buried the few meters worth looking at. Length carries the same ranking the segments did, so colour is still not the only carrier, and the figure and the words stay in the panels. `StatusMark` (panels, lists, legend) and `buildTick` (both renderers) are the one implementation; `SEGMENTS`, `SEGMENT_STEP` and `litSegments` are deleted. Zoom scaling now floors at 0.85 rather than 0.62, below which a mark this small stops being visible, and touch pointers get a 10px hit area. Bands, ratio and everything else from (5) are unchanged. Cache v24. Verified on the logged-in preview: Zone 3A on 17 Sep draws 32 marks — 24 normal, 4 elevated, 2 high, 1 zero, 1 no reading. |
| Water — Hand Readings | `/water` (Hand Readings mode) | ✅ Live 2026-09-08 — migration applied to the Supabase project, Kalhat history loaded (PR #81 awaiting merge) | `irrigation_meters` + `irrigation_daily_readings` (new, separate) · `water_manual_meters` + `water_manual_readings` → trigger → `water_daily_consumption` | Fourth `SegmentedControl` mode holding **two separate tables under `Tabs`: Irrigation and Potable water**, both recorded **per day** (owner ruling 2026-09-08: one new section, two tables, daily entry). **The stored figure is the day's CONSUMPTION in m³, as Kalhat records it** — settled from the owner's file `Muscat_Bay_Water_Consumption_Restructured_5.xlsx` on 2026-09-08 (the first cut assumed cumulative meter indexes and derived today − yesterday; that derivation is gone). A day with no row is "not recorded" and renders "—", never 0; negatives are kept and flagged. One `DateStepper` (capped at today) serves both tabs. Each tab: `StatsGrid` of the **selected day** (three meter groups + "meters recorded n / N", month-to-date in the subtitle) → entry card (consumption per meter + note) → daily log (meters × every day of the month, CSV; a missed day is a visible gap) → daily-consumption chart. **Unified with the app's other visualisations on 2026-09-08 (owner request):** `ChartFrame` gradient areas — `type="natural"` with a per-series `linearGradient` (0.38/0.22 → 0 stops), the same treatment as STP Load-vs-Recovery and the Electricity consumption chart — plus a dashed amber average `ReferenceLine` and a `Max` `ReferenceDot` on the leading series, both computed over RECORDED days only and captioned as such. Both tables use the app-wide body rule `[&>tr]:h-11 [&>tr:nth-child(even)]:bg-component` (44 px rows, zebra on even rows) from the migrated Water Monthly ledgers, over the shared purple `thBase` header. Gaps are never joined. **Irrigation:** registry of 13 meters — the 10 rows on the Kalhat sheet (bore well, STP/TSE into the main tank, main outlet, Central Park TSE = Tank 02, Z5 = Tank 03, Z8 = Tank 04, Village Square = Tank 06, Controller 09, Jumarieh feeding tank, and **"SA TSE", which is on the sheet but not on the owner's meter list — kept under its sheet name until the owner says which tank it is**) plus three from the owner's list with no sheet row yet (PO line, Tank 05 FM, Controller 08). Loaded Jan 2026 – 1 Sep 2026: 2,328 day-rows; one text cell ("New Meter", outlet 13 Feb) skipped; 9 negative days on SA TSE in June kept and flagged. Never touches the potable tables. **Potable water:** the eight bulk meters Kalhat reads — Main Bulk C43659, zone bulks 3A 4300343 · 3B 4300344 · 5 4300345 · 8 4300342 · VS 4300335 · FM 4300346, and "Central park" = the existing Irrigation Tank 02 meter 4300320. Loaded 6 May 2026 onward. **In live daily use since 2026-09-08** — an operator (role granted 2026-09-08) is entering the days through the page itself, and the tables now hold 691 potable and 2,423 irrigation day-rows running to 9 September, with 12 irrigation meters recorded in September including IR Tank 05 and Controller 08, which no spreadsheet had ever carried. This is the feature working as intended: the app, not a workbook, is now the point of entry. **`sql/data/hand-readings-2026-09.sql` is therefore superseded** (2026-09-12) — it was generated from `Muscat_Bay_Water_Consumption_Restructured_6.xlsx` on 8 September as a one-off bridge, and its conflict clause was changed from `do update` to **`do nothing`** so it can never overwrite a figure an operator typed. Six September days disagree between the workbook and what was entered in the app (IRR-MAIN-OUT 2 Sep, IRR-MAIN-TSE 3 and 4 Sep, IRR-TANK-04 2 Sep, IRR-TANK-06 2 Sep, IRR-TANK-JMB 5 Sep); the file lists them, and which side is right is a question for Kalhat, not for the file to settle. The trigger `water_manual_readings_apply` copies each figure into the day cell of the wide daily table, so Zone Watch, the loss balance, the Monthly month-to-date fallback, Satellite and the dashboard read it with **no read-path change**. Overwrite rules (asserted in `sql/tests/manual-readings.test.sql`): `manual_owned` meters (C43659, 4300342, 4300320 — the initializer's EMAIL_OWNED set) always rewrite their cell; the other bulks fill an **empty** cell only, or one still holding the value this table wrote (`applied_consumption`). On the live load 268 rows went into the daily table (the three owned meters — values identical to what the owner had already keyed by SQL) and 359 rows stayed in `water_manual_readings` only because Grafana already held an instrumented value for those zone-bulk days; those Grafana values differ from Kalhat's (e.g. Zone 5, 6 May-26: 108.5 vs 165) and are deliberately not replaced. Writes go through `saveManualReadingsAction` with the caller's own session, so RLS decides (operator+ may write; viewer sees the cards read-only with a caption). Own fetch + realtime channel per tab, reporting into the page's single `StatusChip`. Still open: `npm run screenshots` (needs a running app + login); the entering colleague's `profiles.role` must be set to `operator`. |
| Electricity | `/electricity` | ✅ Live, inspection-first redesign 2026-07-05 | `electricity_meters` / `electricity_readings` | Two tabs (down from three): **Load Watch** (default) — category (meter_type) severity cards worst-first, category×month load heatmap, auto Exceptions & Actions register (per-meter spike/dip/zero/negative/missing vs each meter's own baseline), with KPIs + trend charts kept below; **Meters & Data** (Analysis + Database in one tab). **2026-07-06:** de-duplicated after review — the two near-identical horizontal bar charts (top-10 consumers + meter-vs-average) collapsed into one **Meters by Consumption** chart (ranked high→low, each bar colored above/below the group average with an "Avg" reference line, the filter-selected meter outlined); the two overlapping consumption tables (Monthly Breakdown + the separate anomaly grid) collapsed into one **Meter Consumption & Anomalies** table that follows the shared date range and keeps search + type filter + rows pagination + CSV, now with a Cost column, a Total row and the anomaly tinting on each cell. Monthly readings through Jun-26 (loaded via hand-run `sql/migrations/update_electricity_*.sql`) **2026-08-31:** KPI tiles swapped from `StatsGrid` to the Water tile (`HierarchyStatGrid`, promoted out of `components/water/daily-report/inline-shared.tsx` into `components/shared/hierarchy-stat-card.tsx`) — same `StatItem[]`, same column counts, so the swap is one line and nothing reflows. The tile is quieter by design (no trend arrow or status word) because Load Watch carries that detail beneath it; `StatsGrid` stays where the trend is the message. **2026-09-03:** the Load Watch "Load briefing" strip (one wide card, four columns) is now a `StatsGrid` row of the same four figures — Consumption, Estimated cost, Previous month, Flagged meters — so Electricity's headline row is the same tile as Water, STP and HVAC (owner: "wow very identical"); the month moved from the strip caption into the first tile's subtitle, cost turns `danger` only above the configured budget, flagged meters are `danger`/`success`. Later the same day the category table's **"6-month trend" column** (sparkline + % vs previous month, and its sort key) was removed at the owner's request; the trend still reaches the CSV export, and the phone-width summary line keeps cost only |
| STP Plant | `/stp` | ✅ Live, inspection-first redesign 2026-07-05 | `stp_operations` | Two tabs: **Plant Watch** (default) — process-health cards worst-first (efficiency, hydraulic load, TSE reuse, tankers, data completeness), load-vs-recovery chart, metric×day heatmap, auto Exceptions & Actions register (data-relative severity + efficiency bands); **Operations & Trends** keeps the KPIs, charts, daily log/CSV and folds the Airtable DB into a collapsible. Daily ops through May-26; 3D plant twin exists only on unmerged PR #21. `stp_operations` added to the `supabase_realtime` publication 2026-07-06, so both this page and the dashboard now refresh live on data changes (the page already subscribed but the table was unpublished) **2026-08-31:** Load vs Recovery rebuilt as an interactive area chart — two layered gradient bands (inlet, TSE) on a single m³ axis plus a 90/30/7-day window selector, replacing the bars-plus-efficiency-line pair on two axes. The bands are layered, not stacked: TSE is the portion of the inlet that came back out, so it sits inside the inlet band and the gap between them is the unrecovered water. Efficiency moved to the tooltip label (it keeps its own process-health tile) — a percentage never shared a scale with m³. TSE stays raw, so a negative reading dips below the axis and a TSE band above the inlet band shows >100% recovery, a physical impossibility the paired axes used to hide. KPI tiles swapped from `StatsGrid` to the Water tile (`HierarchyStatGrid`) so water, electricity and STP present headline figures identically. **2026-09-02:** swapped back to `StatsGrid` — the owner ruled that every module's KPI row is the HVAC tile (`DESIGN_SYSTEM.md` §7); `components/shared/hierarchy-stat-card.tsx` is now unreferenced and kept pending approval to delete |
...
- **Hand readings (added 2026-09-08, migration `20260908_manual_meter_readings.sql`,
  applied live the same day):** Kalhat's daily consumption figures for the eight
  potable bulk meters land in `water_manual_readings`; the trigger copies each
  day into the matching `water_daily_consumption.day_N` cell. Three
  accounts are `manual_owned` (C43659, 4300342, 4300320 — the initializer's
  EMAIL_OWNED set) and are always rewritten; the rest fill empty cells only and
  never overwrite Grafana. Because the fill lands in the same wide table, every
  existing reader (Daily, month-to-date fallback, Satellite, dashboard) sees it
  unchanged. Irrigation is a separate pair of tables (`irrigation_meters`,
  `irrigation_daily_readings`) and never touches the potable data. Both readings
  tables are added to the realtime publication. **If the 2026-09-01 security
  migration is ever re-run, add the four new tables to its inventory first** —
  its fail-closed sweep strips policies from unlisted tables. Local proof:
  `sql/tests/run-local.sh` (PostgreSQL 16, throwaway cluster).
...
- **Open 2026-09-10 — real unaccounted water upstream of every zone, ~20–40% of
  ...
  | Month | Days | NAMA L1 /day | ΣL2 + ΣDC /day | Gap /day | Gap % |
  |---|---|---|---|---|---|
  | Jun-26 | 24 | 1,932 m³ | 1,146 m³ | 786 m³ | 40.7% |
  | Jul-26 | 31 | 1,491 m³ | 992 m³ | 499 m³ | 33.5% |
  | Aug-26 | 31 | 1,235 m³ | 960 m³ | 275 m³ | 22.2% |
  | Sep-26 | 9 | 1,358 m³ | 978 m³ | 380 m³ | 28.0% |

  Four findings, each reproducible from `water_daily_consumption`:
  1. **The gap is real, not a reading artefact.** Every one of the 71 rolling
     7-day windows is positive, floor 15.8%. Timing noise cancels over a week;
     this does not. Aug-26 daily L1 sums to 38,285 m³ against the NAMA monthly
     bill of 38,291 m³ in `water_monthly_consumption` — the daily L1 series *is*
     the billed meter, so this is water Muscat Bay pays for and never measures.
  2. **It is not consumption.** The measured network is extremely stable —
     1,020 ± 124 m³/day (CV 12%) — while L1 swings 934 → 2,904 m³/day (CV 28%).
     Regressing the gap on the network below gives **r² = 0.00**; regressing it
     on L1 gives **r² = 0.90**. The gap lives entirely on the supply side.
  3. **Day-to-day swings are a reading-basis mismatch, not real variation.** L1
     is a whole number on 101 of 101 days; ΣL2 is a whole number on 7 of 101.
     Different metering systems, different read times — which also produces the
     three days where L1 came in *below* the network. **Read this balance in
     7-day windows, never single days.**
  4. **It is falling**: 40.7% → 33.5% → 22.2% across Jun–Aug. A fixed leak does
     not halve itself; this points at a demand-driven offtake (tanker filling,
     construction, irrigation-tank top-up in peak summer) or something that was
     repaired, rather than a single burst main.

  **Not yet diagnosed — the decisive test is minimum night flow on the main.**
  At 02:00–04:00 legitimate demand should be ≈5–8 m³/h against the measured
  1,020 m³/day. A continuous leak keeps running at 3am; a tanker point or
  construction standpipe does not. If the NAMA meter is on the NEC Oman Grafana
  feed at sub-daily resolution, this is answerable from existing telemetry with
  no field work. Failing that: night step-test by closing zone bulk valves one
  at a time, a walk of the trunk from the NAMA chamber listing every unmetered
  tee, and a clamp-on ultrasonic check of L1 against its own totaliser.
- **Open 2026-09-10 — the ZEN Project zone has no history before Aug-26.** The
  bulk (`4300348`, "Al Adrak Construction") carried daily readings from Jan-26,
  but its 81 child meters (`4300351`–`4300431`) only exist from Aug-26. For
  Jan-26 to Jul-26 the zone therefore reads as 100% loss (bulk with ΣL3 = 0),
  which is honest — the children genuinely were not metered — but it does lift
  the network loss figure for those months. Do not read those months' ZEN loss
  as a leak.
...
- **Open 2026-09-08 — hand readings, two loose ends.** (1) "SA TSE" on the
  ...
  FM? Controller 08?) — renaming is a one-row registry update. (2) The
  entering colleague's `profiles.role` must be set to `operator` — the default
  `viewer` role sees the entry cards read-only by design. Also: the Kalhat
  zone-bulk figures differ from Grafana's for the same days (Zone 5 on
  6 May-26: 165 hand vs 108.5 Grafana); both are kept, Grafana in the daily
  table and Kalhat in `water_manual_readings`, and the daily views show the
  Grafana value. Screenshots (`npm run screenshots`) still not produced (no
  running app / login in the build session).
...
- **Design-system migration in progress (2026-09-02).** Only Water is on
  ...
  alias: the kit's `--color-muted` is the muted *text* colour (`text-muted`),
  so the legacy pages' `bg-muted` surface was renamed `bg-muted-bg` (a pure
  rename, no visual change). The shadcn `button.tsx` / `badge.tsx` remain
  beside the kit's `mb-button.tsx` / `mb-badge.tsx` for the same reason. The
  design lint (`eslint.design-rules.mjs`) is scoped to `app/water/**` and
  `components/water/**`; each migrated page adds its folders. Two Water files
  are no longer referenced by the page but are kept pending the owner's
  approval to delete: `components/water/date-range-picker.tsx` (still has
  its test) and `components/shared/period-filter-panel.tsx`. Three legacy
  pages (`/pest-control`, `/forgot-password`, `/auth/callback`) used
  `text-muted` while it still meant a light *surface* colour — their captions
  now render in the intended muted grey. **Backlog from the 2026-09-03 QA
  audit (each item belongs to that page's migration session, not to a
  one-off fix):** (a) five card treatments and three corner radii coexist —
  `SectionCard` 10.5 px on Water, `.card-elevated` 7 px on Dashboard/STP/
  Electricity/Settings/Pest, shadcn `Card` 12 px on Fire/HVAC, raw
  `rounded-[10.5px]` on Contractors/Assets; (b) STP is the only page whose
  top-level strip is `variant="secondary"` — under the §7 ruling every
  single-strip page should be, so Electricity, HVAC, Contractors, Assets, Fire
  and Settings switch when they migrate; (c) the KPI row sits above the tabs
  (Contractors), below them (STP) or inside one tab (HVAC, Assets, Fire,
  Electricity) — `DESIGN_SYSTEM.md` §5 says tabs → StatsGrid; (d) load
  failures on Electricity, STP, Assets and Fire surface only as small text
  under the status chip with no Retry, and STP has no empty-table state;
  (e) the dashboard shows two live/offline indicators (deck badge + ticker)
  and Pest Control none; (f) page titles differ from the sidebar names
  ("Electricity Monitoring", "Contractor Management", "Assets Register",
  "Fire Safety Management"); (g) two page-header and two breadcrumb components
  exist until the last page migrates; (h) `components/shared/hierarchy-stat-card.tsx`
  is now referenced by nothing (kept pending approval to delete); (i) the
  Recharts "width(-1) and height(-1)" dev-console warning fires on every chart
  rendered with `height="100%"` (dashboard, Electricity, Water daily) — cosmetic
  in production, but a chart-plumbing fix, not a design one. Arbitrary-value
  counts per page (migration effort, highest first): Electricity 54, STP 52,
  Fire 47, Contractors 38, HVAC 35, Dashboard 28, Assets 23, Pest 4, Settings 0,
  Water 0. One phone-width limitation is by design: the kit `PageHeader` keeps
  a fixed height and truncates its one-line description below ~640 px.
- **Closed 2026-09-03 — Water Daily cumulative balance chart removed.** It
  counted a missing L2 reading as 0 supply (`buildZoneMtd`, `cs += p.l2 ?? 0`
  while the day's ΣL3 was still added) and was the running sum of the "Zone
  daily consumption trend" chart above it. The owner chose removal; the
  month-to-date loss now lives in the trend card's footer, computed only over
  days with a bulk reading, and the freed half of the row carries the new
...
- **#79 Design System v2.0 — foundation + Water (2026-09-02).** Branch
  ...
  earlier hardening PR was closed). Kept as a **draft through six review
  rounds on the Vercel preview** in light and dark mode
  (`muscatbay-git-claude-supabase-servic-76af09-arahim900s-projects.vercel.app/water`;
  1440 px light + dark captures in `muscatbay/app/screenshots/`); on
  2026-09-03 the owner delegated the merge decision ("you decide"), so the PR
  was taken out of draft for the automated review and merged (squash) once
  that review had run — the merge is what the next Development-log entry
  records. What it
  ...
  once from `globals.css`, DM Sans loaded in `app/layout.tsx` (Inter retired),
  the twelve primitives in `components/ui/` with a barrel, `lib/cn.ts`, the
  design lint wired into `eslint.config.mjs` (scoped to the Water folders),
  `npm run screenshots` (Playwright, 1440 px, light + dark), and the "Design
  rules for agents" section pasted verbatim into `CLAUDE.md`. Then the first
  page migration — **Water**, Monthly + Daily + Satellite — described in the
  §2 rows above. Deliberate deviations, each explained in the PR: the kit's
  `KpiCard` `any` became `ElementType` (repo lint forbids `any`); the kit's
  ESLint rule file gained a named export (repo lint); legacy pages keep their
  old tokens behind a `bg-muted-bg` alias (§4); the Water daily briefing
  ticker became a static card (kit rule 7); the old WCAG-checked focus-ring
  rule in `globals.css` still outranks the kit's teal outline. Gates:
  TypeScript, ESLint (design rule included), 322 Vitest tests, production
  build, mobile `tsc` — all clean. **Nothing in this PR touches the
  database.** **Round 2 (owner review of the preview, same day):** the
  kit's new looks were judged inconsistent with the rest of the app, so the
  primitives now wrap the app-wide `TabNavigation` strip and `StatsGrid`
  tile, the period control is restyled as pressable controls, the donut
  matches HVAC, the blue month-to-date note is gone, STP is back on
  `StatsGrid`, and the rulings are written into `DESIGN_SYSTEM.md` §7 and
  `CLAUDE.md`. **Round 3 (multi-agent QA, 2026-09-03):** a static
  consistency audit of all ten pages plus a Playwright crawl of every route in
  both themes at 1440 and 390 px (52 captures, a click on every tab). Clean on
  the hard checks — no page errors, no horizontal overflow at either width, one
  heading and a breadcrumb per route, every tab click lands. Fixed in the PR:
  the Water mode switch used to disappear when the monthly read failed, locking
  the operator out of Daily and Satellite — the switch now stays and only the
  two views fed by the monthly fetch show the error/empty state; the Water
  balance flow was clipped at phone width (`justify-center-safe`); the Daily
  tab panels had no ids; Contractors, HVAC and Fire Safety tabs pointed at
  panels that did not exist and Electricity's data tab at the wrong id (one
  `role="tabpanel"` wrapper per page now, page-specific tab-strip labels on all
  six pages); water-blue KPI tiles on the Assets and Fire pages became
  `primary`/`info`; the Electricity Load Watch "Load briefing" strip became a
  `StatsGrid` row of the same four figures after the owner compared the three
  KPI rows side by side. **Round 5 (2026-09-03, owner's second pass):** the
  Electricity category table lost its "6-month trend" column at the owner's
  request; `SectionBoundary` now recognises a chunk-load failure (a tab still
  running the previous build after a deployment asks for files that no longer
  exist) and reloads the page once instead of offering a Retry that re-requests
  the same missing file — the "HVAC Overview could not be displayed" report was
  this, not a data fault: the Overview is a `next/dynamic` section, the live
  tables were checked (361 findings, 22 recurring issues, 6 contracts, 19
  correspondence rows, no nulls in the fields the page reads) and nothing in
  the component's own render path can throw on that data. The Water Zone
  Analysis "Month-to-date cumulative balance" chart was reviewed at the
  owner's request (see the Water — Daily row and §4). **Round 6 (2026-09-03,
  owner's decision on that chart):** the cumulative chart is gone; "Zone daily
  consumption trend" moved to the right half of the row at the half-width
  chart height (260 px) with the month-to-date balance in its footer, and the
  ...
  different ways, error states without Retry on Electricity/STP/Assets/Fire, two
  live indicators on the dashboard and none on Pest Control, and the Recharts
  "width(-1)" dev warning shared by every `height="100%"` chart.

### 5a. Archived — 2026-09-02 hardening wave (pushed to `main` as `98375c0`, reverted in `5620c4a`)

> The entries below describe application work that reached `main` on
> 2026-09-02 and was **reverted the same day at the owner's request** (design
> and behaviour changes he had not asked for). They are kept as history only:
> none of that code is in the tree. The **database-side security migrations
> stay applied** to the live project — see §4.
...
- **Pre-deployment remediation wave (2026-09-02)** — same branch as #77's
  follow-up. A five-phase readiness list plus a UI/UX standardisation brief was
  audited item-by-item against the codebase before any code was written. Two
  phases needed nothing: **Phase 2 (data accuracy)** and **Phase 4 (embeds,
  CSP, indexing)** were already fully implemented — fire PPM statuses compute
  from the current date, STP flags TSE-above-inlet and recovery-above-100%,
  contract-date conflicts are reconciled, `acknowledged_at`/`resolved_at` are
  separate and the UI reads open incidents from the database, and
  `lib/embed-security.ts` + `proxy.ts` already enforce an iframe allowlist and a
  nonce/`strict-dynamic` production CSP with `robots.txt` and `X-Robots-Tag`.
  What was genuinely outstanding, and is now done:
  - **RLS is tested by CI.** `npm run test:sql:local` existed but ran only when
    someone remembered. A `sql-security` job in `.github/workflows/quality.yml`
    now applies both migrations to a throwaway PostgreSQL 16 cluster on every
    PR, plus weekly — policy rot changes nothing in the app, so no ordinary PR
    would catch it. Kept separate from `verify` so a lint failure cannot mask a
    broken access-control policy.
  - **A missing accent-as-text token.** `--secondary` (`#A1D1D5`) is a
    background tint, but eight call sites used it as *text* on light surfaces —
    about **1.6:1** against a 4.5:1 AA floor, unqualified by theme, so light mode
    was the broken one. The `--mb-*-text` convention existed for status colours
    but had no accent equivalent, which is why call sites reached for the raw
    teal. New **`--mb-secondary-text` (`#37767B`)**, derived the way
    `BRAND_DESIGN.md` §2.6 derives the status icon tier (brand hue and
    saturation held, lightness lowered): **4.89:1** on `--background`, **5.20:1**
    on `--card`, asserted by a unit test. `--ring` was not reusable — at 3.54:1
    it clears the 3:1 non-text floor only. Applied to the mobile bottom nav
    (highest traffic — the active tab on every route), topbar role label,
    secondary tab icon, Electricity meter chip and Fire Safety labels.
  - **Nine unnamed data tables** (5 Assets, 3 Contractors, 1 Fire Safety) gained
    screen-reader captions naming their real columns and stating that a blank
    means *not recorded*, so a missing value is never heard as a zero. Two
    existing `aria-label`s described tables they were not, and `aria-label`
    **beats** `<caption>` in the accessible-name computation — so those were
    renamed too; a caption alone would have left the wrong name announced.
  - **Three icon-only controls under 44px on touch** (bottom-nav close, export
  ...
  - **Charts bypassing the shared container.** HVAC and Fire Safety imported
    Recharts' `ResponsiveContainer` directly; they survived only because a
...
- 2026-09-10 — feat(water): gradient supply trend with loss bars, collapsible balance table (#84)
