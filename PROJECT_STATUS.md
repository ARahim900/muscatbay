...
| Water — Monthly | `/water` (Monthly tab) | ✅ Live, fully dynamic | `water_meters` + `water_monthly_consumption` | Balance A1/A2/A3, zones, buildings, DC, exceptions; months appear automatically (see §3). **2026-09-02 — brought onto the shared module pattern.** Water was the outlier of the three utility modules: Electricity and STP render their KPI grid and briefing tab-agnostically, while Water gated its four KPI cards behind the *Overview* sub-tab, so moving to Zone Analysis or Exceptions took the headline figures off screen. The grid now renders on all five Monthly sub-tabs. A **Water briefing** was added (the shared `OperationalBriefing`, as Load Watch and Plant Watch use) — it previously existed only on the Daily tab, leaving the module's default landing view as the one place with no summary; its four findings are derived from the page's existing `computePeriod` output, never hardcoded. Nine charts moved from the local `Panel` wrapper to the shared **`ChartShell`**, gaining per-chart loading/empty/error states and a management interpretation line, so a single chart with no data degrades on its own instead of relying on the section boundary; the nine non-chart Panels (gauges, tables, heatmap, register) deliberately stay on `Panel`. The local `Kpi` tile now matches the shared `StatsGrid` tile — top accent bar, icon top-right, trend bottom-right, `shadow-card-standard` instead of an inline `boxShadow`. Honesty guard worth noting: `pct()` returns `0` on a zero denominator, so a period with no A1 volume would have rendered a confident "0% loss"; `briefing-metrics.ts` returns `null` there and the tile shows "—" with the reason, and a negative loss (A3 > A1) gets its own branch rather than reading as "within target". **2026-07-06:** Zone Analysis now surfaces the **primary/trunk network (A1→A2)** as a first-class item — the loss *above* every zone that the per-zone drill-downs structurally can't show. It has a top-of-section banner, its own dropdown option + drill-down (KPIs, main-bulk-vs-reached bar, monthly A1-vs-A2 trend, and an A1 reconciliation table of Σ zone-bulk + Σ direct vs main bulk), plus an Exceptions-register row when the A1→A2 gap breaches target or goes negative (A2>A1 → main-meter/timing issue). Live A1→A2 gap has run 0–41% month-to-month (even negative in Jan/Feb-26), so it is material and reading-sensitive. The long tables (A1 reconciliation, individual meters, exceptions register, main database) now carry a **10/20/50/All rows selector** so operators cap what shows instead of scrolling a fixed box (drill-down tables default to 20; the database defaults to All) |
