#!/usr/bin/env node
/**
 * DC Meters Q1 2026 Upload Script
 * Upserts daily consumption data for 11 Direct Connection meters × 3 months = 33 records
...
function parseCSV(content) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (!line) continue;
        // Handle quoted CSV fields (some labels contain commas)
        const values = [];
        let current = '';
        let inQuotes = false;
...
async function uploadCSV(csvPath) {
    ...
    if (records.length > 0) {
        ...
        console.log('  Sample: ' + sample.meter_name + ' (' + sample.account_number + ') ' + sample.month + ' day_1=' + sample.day_1);
    }

    // Upsert in one batch (only 11 records per file)
    const { error } = await supabase
        .from('water_daily_consumption')
        .upsert(records, { onConflict: 'account_number,month,year' });
    ...
    console.log('  ' + records.length + ' records upserted successfully');
    return records.length;
}

async function main() {
    console.log('DC Meters Q1 2026 Daily Consumption Upload');
    console.log('='.repeat(50));

    const csvFiles = [
        path.join(__dirname, 'dc_meters_jan_26.csv'),
        path.join(__dirname, 'dc_meters_feb_26.csv'),
        path.join(__dirname, 'dc_meters_mar_26.csv'),
    ];
