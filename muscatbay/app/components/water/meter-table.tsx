        ...
        <div className="space-y-4">
            ...
            <div className="hidden md:block overflow-x-auto rounded-xl border border-slate-200/80 dark:border-slate-700/80 bg-white dark:bg-slate-900 shadow-sm">
                <table className="w-full text-sm">
                    <thead>
                        <tr className="bg-slate-100/60 dark:bg-slate-800/70">
                            <th className="text-left py-4 px-5 font-semibold text-sm text-slate-600 dark:text-slate-300 border-b border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-200/60 dark:hover:bg-slate-700/60 transition-colors" {...sortHeaderProps('label')}>
                                <div className="flex items-center gap-1.5">Meter Label <ArrowUpDown className="w-3 h-3" /></div>
                            </th>
                            <th className="text-left py-4 px-5 font-semibold text-sm text-slate-600 dark:text-slate-300 border-b border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-200/60 dark:hover:bg-slate-700/60 transition-colors" {...sortHeaderProps('account')}>
                                <div className="flex items-center gap-1.5">Account # <ArrowUpDown className="w-3 h-3" /></div>
                            </th>
                            <th className="text-center py-4 px-5 font-semibold text-sm text-slate-600 dark:text-slate-300 border-b border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-200/60 dark:hover:bg-slate-700/60 transition-colors" {...sortHeaderProps('level')}>
                                <div className="flex items-center justify-center gap-1.5">Level <ArrowUpDown className="w-3 h-3" /></div>
                            </th>
                            <th className="text-left py-4 px-5 font-semibold text-sm text-slate-600 dark:text-slate-300 border-b border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-200/60 dark:hover:bg-slate-700/60 transition-colors" {...sortHeaderProps('zone')}>
                                <div className="flex items-center gap-1.5">Zone <ArrowUpDown className="w-3 h-3" /></div>
                            </th>
                            <th className="text-left py-4 px-5 font-semibold text-sm text-slate-600 dark:text-slate-300 border-b border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-200/60 dark:hover:bg-slate-700/60 transition-colors" {...sortHeaderProps('parentMeter')}>
                                <div className="flex items-center gap-1.5">Parent Meter <ArrowUpDown className="w-3 h-3" /></div>
                            </th>
                            <th className="text-left py-4 px-5 font-semibold text-sm text-slate-600 dark:text-slate-300 border-b border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-200/60 dark:hover:bg-slate-700/60 transition-colors" {...sortHeaderProps('type')}>
                                <div className="flex items-center gap-1.5">Type <ArrowUpDown className="w-3 h-3" /></div>
                            </th>
                            {displayMonths.map(m => (
                                <th key={m} className="text-right py-4 px-5 font-semibold text-sm text-slate-600 dark:text-slate-300 border-b border-slate-200 dark:border-slate-700 cursor-pointer hover:bg-slate-200/60 dark:hover:bg-slate-700/60 transition-colors" {...sortHeaderProps(m)}>
                    ...
                    <tbody>
                        {paginatedMeters.map((meter) => {
                            const levelColors = LEVEL_COLORS[meter.level] || LEVEL_COLORS['N/A'];
                            return (
                                <tr key={meter.accountNumber} className="border-b border-slate-100/80 dark:border-slate-800/80 odd:bg-white dark:odd:bg-transparent even:bg-slate-50/50 dark:even:bg-slate-800/20 hover:bg-primary/5 dark:hover:bg-primary/10 transition-colors">
                                    <td className="py-4 px-5 font-medium text-sm text-slate-800 dark:text-slate-200">{meter.label}</td>
                                    ...
                                    {displayMonths.map(m => {
                                        const val = getConsumption(meter, m);
                                        return (
                                            <td key={m} className="py-4 px-5 text-right font-medium text-sm text-slate-700 dark:text-slate-300">
                                                {val > 0 ? val.toLocaleString('en-US') : '-'}
                                            </td>
                                        );
                                    })}
