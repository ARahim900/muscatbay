...
// Skeleton for data tables (standalone, not inside a <table>)
...
// Skeleton rows for use inside a <TableBody> (renders valid <tr>/<td> elements)
export function TableBodySkeleton({ rows = 5, columns = 4 }: { rows?: number; columns?: number }) {
    return (
        <>
            {Array.from({ length: rows }).map((_, rowIndex) => (
                <tr key={rowIndex}>
