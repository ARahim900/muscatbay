  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <span className="text-sm text-muted-foreground dark:text-muted-foreground ml-auto">
          <span className="font-semibold text-foreground dark:text-muted-foreground/70">{filtered.length}</span> equipment
        </span>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="py-12 text-center text-muted-foreground dark:text-muted-foreground bg-white dark:bg-muted rounded-xl border border-border dark:border-border">
          <HardDrive className="h-10 w-10 mx-auto mb-3 text-muted-foreground/70" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map((e) => {
            ...
            return (
              <Card
                key={e.id}
                className={`bg-white dark:bg-muted rounded-xl shadow-sm hover:shadow-md motion-safe:hover:-translate-y-0.5 transition-[transform,box-shadow] duration-200 ${
                  isNotOp
              ...
              >
                <CardContent className="p-4 space-y-2.5">
                  <div className="flex items-center justify-between gap-2">
                    <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-medium bg-primary/10 text-primary dark:bg-primary/20 dark:text-muted-foreground/70">
                      {e.building}
                    </span>
                    <div className="flex items-center gap-1.5">
                      <div className={`w-2.5 h-2.5 rounded-full ${getStatusDot(e.operational_status)}`} />
                      <span className="text-[10px] text-muted-foreground dark:text-muted-foreground">{e.operational_status}</span>
                    </div>
                  </div>
                  <p className="font-semibold text-sm text-foreground dark:text-muted-foreground truncate">
                    {e.equipment_label}
                  </p>
                  ...
                  {e.notes && (
                    <p className="text-[11px] text-muted-foreground dark:text-muted-foreground line-clamp-2">{e.notes}</p>
                  )}
                </CardContent>
              </Card>
            );
