...
// Firefighting Data - Based on BEC AMC Contract (41 FA Panels, ~549 Extinguishers, 30 Hydrants, 25 Hose Reels, 5 Pumps)
const generateFireEquipment = (): FireSafetyEquipment[] => {
    const equipment: FireSafetyEquipment[] = [];
    const inspectors = ["Arun Achuthan", "Nadim Ahmad", "Suneesh L", "FA Team"];
    const zones = ["Zone 1", "Zone 2", "Zone 3", "Zone 4", "Zone 5", "Staff Accom", "Village Square", "Technical"];
    const locations = [
        "FM Office Building", "Experience Centre", "Staff Accom Bldg 1", "Staff Accom Bldg 2",
        "Staff Accom Bldg 3", "Staff Accom Bldg 4", "Staff Accom Bldg 5", "Staff Accom Bldg 6",
        "Staff Accom Bldg 7", "Staff Accom Bldg 8", "Security Room", "Nursery", "Control Room",
        "Taxi Building", "ROP Building", "Technical Building", "STP Building", "Village Square",
        "Apartment 44", "Apartment 45", "Apartment 46", "Apartment 47", "Apartment 48",
        "Apartment 49", "Apartment 50", "Apartment 51", "Apartment 52", "Apartment 53",
        "Apartment 54", "Apartment 55", "Apartment 56", "Apartment 57", "Apartment 58",
        "Apartment 59", "Apartment 60", "Apartment 61", "Apartment 62", "Apartment 74", "Apartment 75"
    ];
    ...
    // FA Panels (41 total)
    for (let i = 0; i < 41; i++) {
        const status = i < 38 ? "Operational" : (i === 38 ? "Needs Attention" : "Maintenance Due");
        const priority = status === "Operational" ? "Medium" : "High";
        equipment.push({
            id: `fa-${id++}`,
            name: `Fire Alarm Panel ${i + 1}`,
            location: locations[i % locations.length],
            status: status as FireSafetyEquipment["status"],
            priority: priority as FireSafetyEquipment["priority"],
            battery: Math.round(75 + Math.random() * 25),
            signal: Math.round(85 + Math.random() * 15),
            next_maintenance: `2026-0${Math.floor(Math.random() * 4) + 4}-${String(Math.floor(Math.random() * 28) + 1).padStart(2, '0')}`,
    ...
    // Fire Extinguishers (sample 60 out of ~549)
    const extTypes = ["DCP 4.5 KG", "DCP 6 KG", "DCP 9 KG", "CO2 5 KG", "CO2 2 KG", "Foam 9L"];
    for (let i = 0; i < 60; i++) {
        const statusRoll = Math.random();
        const status = statusRoll > 0.15 ? "Operational" : (statusRoll > 0.08 ? "Maintenance Due" : (statusRoll > 0.03 ? "Needs Attention" : "Expired"));
        const priority = status === "Expired" ? "Critical" : (status === "Needs Attention" ? "High" : "Medium");
        equipment.push({
            id: `fe-${id++}`,
            name: `Extinguisher ${extTypes[i % extTypes.length]} - ${i + 1}`,
            location: locations[i % locations.length],
            status: status as FireSafetyEquipment["status"],
            priority: priority as FireSafetyEquipment["priority"],
            battery: null,
            signal: null,
            next_maintenance: `2026-0${Math.floor(Math.random() * 6) + 1}-${String(Math.floor(Math.random() * 28) + 1).padStart(2, '0')}`,
            inspector: inspectors[i % inspectors.length],
    ...
    // Hose Reels (25 total)
    for (let i = 0; i < 25; i++) {
        const status = i < 23 ? "Operational" : "Maintenance Due";
        equipment.push({
            id: `hr-${id++}`,
            name: `Hose Reel ${i + 1}`,
            location: locations[i % locations.length],
            status: status as FireSafetyEquipment["status"],
            priority: status === "Operational" ? "Medium" : "High",
            battery: null,
            signal: null,
            next_maintenance: `2026-0${Math.floor(Math.random() * 4) + 4}-${String(Math.floor(Math.random() * 28) + 1).padStart(2, '0')}`,
            inspector: inspectors[i % inspectors.length],
    ...
    // Fire Hydrants (30 total - 27 external + 3 staff)
    for (let i = 0; i < 30; i++) {
        const status = i < 28 ? "Operational" : "Needs Attention";
        equipment.push({
            id: `fh-${id++}`,
            name: `Fire Hydrant ${i < 3 ? "Staff-" : "Ext-"}${i + 1}`,
            location: i < 3 ? `Staff Accom Bldg ${i + 1}` : `External Zone ${Math.floor((i - 3) / 5) + 1}`,
            status: status as FireSafetyEquipment["status"],
            priority: status === "Operational" ? "Low" : "High",
            battery: null,
            signal: null,
            next_maintenance: `2026-0${Math.floor(Math.random() * 4) + 4}-${String(Math.floor(Math.random() * 28) + 1).padStart(2, '0')}`,
            inspector: inspectors[i % inspectors.length],
            type: "Fire Hydrant",
            zone: i < 3 ? "Staff Accom" : `Zone ${Math.floor((i - 3) / 5) + 1}`
        });
    }

    // Fire Pumps (5 total - 2 Electric, 2 Diesel, 1 Jockey)
    const pumps = [
        { name: "Electric Fire Pump 500 GPM", type: "Electric Pump" },
        { name: "Electric Fire Pump 50 GPM", type: "Electric Pump" },
        { name: "Diesel Fire Pump 500 GPM", type: "Diesel Pump" },
        { name: "Diesel Fire Pump 50 GPM", type: "Diesel Pump" },
        { name: "Jockey Pump 13.5 kW", type: "Jockey Pump" },
    ];
    pumps.forEach((pump, i) => {
        equipment.push({
            ...
            status: i < 4 ? "Operational" : "Maintenance Due",
            priority: "Critical",
            battery: null,
            signal: null,
            next_maintenance: `2026-04-${String(Math.floor(Math.random() * 28) + 1).padStart(2, '0')}`,
            inspector: "Arun Achuthan",
            type: pump.type,
            zone: "Technical"
        });
    });

    // Smoke Detectors (sample 30)
    for (let i = 0; i < 30; i++) {
        const statusRoll = Math.random();
        const status = statusRoll > 0.1 ? "Operational" : (statusRoll > 0.05 ? "Needs Attention" : "Maintenance Due");
        equipment.push({
            id: `sd-${id++}`,
            name: `Smoke Detector SD-${i + 1}`,
            location: locations[i % locations.length],
            status: status as FireSafetyEquipment["status"],
            priority: status === "Operational" ? "Medium" : "High",
...
export const FIRE_EQUIPMENT: FireSafetyEquipment[] = generateFireEquipment();
