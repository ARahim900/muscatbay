...
import { ParsedCsvRow } from "@/types/water-section"
...
const WaterSystemSection = ({ fullView = false }: WaterSystemSectionProps) => {
  // --- THEME Definition ---
  ...
  const [processedData, setProcessedData] = useState<WaterMeterRow[]>([])
  ...
  const [selectedZone, setSelectedZone] = useState<string>("Zone_05")
  ...
  useEffect(() => {
    ...
    const loadAndProcessData = () => {
      ...
      try {
        ...
        const parsedData = (window as any).Papa.parse(consolidatedCsvData, {
          header: true,
          dynamicTyping: true,
          skipEmptyLines: true,
        ...
        setRawData(parsedData.data as ParsedCsvRow[]) // Store raw data
        const dataForProcessing = parsedData.data || []
        const processed = processRawData(dataForProcessing as ParsedCsvRow[])
  ...
  const processRawData = (rawDataArray: ParsedCsvRow[]): WaterMeterRow[] => {
    const allMonths = [
      "Jan-24",
      "Feb-24",
      "Mar-24",
    ...
    return rawDataArray
      .map((row: ParsedCsvRow) => {
        const processedRow: ProcessedWaterData = {
        ...
        return processedRow as WaterMeterRow
      })
  ...
  const calculatedMetrics = useMemo(() => {
    ...
    const totalL1Supply = l1Meter ? l1Meter[selectedPeriod] as number || 0 : 0
    ...
    const l2SumFromL2Labels = l2Meters.reduce((sum, m) => sum + (m[selectedPeriod] as number || 0), 0)
    const l2SumFromDCLabels = dcMetersFromL1.reduce((sum, m) => sum + (m[selectedPeriod] as number || 0), 0)
    ...
    const excludedValue = excludedMeter ? excludedMeter[selectedPeriod] as number || 0 : 0
    const l3SumFromL3Labels = l3MetersForTotal.reduce((sum, m) => sum + (m[selectedPeriod] as number || 0), 0)
    const l3SumFromDCLabels = dcMetersForTotal.reduce((sum, m) => sum + (m[selectedPeriod] as number || 0), 0)
    ...
    const consumptionByType = dataForPeriod
      ...
      .reduce((acc: Record<string, number>, meter) => {
        const value = meter[selectedPeriod] as number || 0
    ...
    const zoneMetrics: Record<string, ZoneMetrics> = {}
    ...
    zonesWithL2Meters.forEach((zoneName) => {
      ...
      const l2BulkReading = zoneL2Meters.reduce((sum, m) => sum + (m[selectedPeriod] as number || 0), 0)
      const l3MetersInZone = dataForPeriod.filter((m) => m.label === "L3" && m.zone === zoneName)
      const l3SumInZone = l3MetersInZone.reduce((sum, m) => {
        let value = m[selectedPeriod] as number || 0
  ...
  const handleRefresh = () => {
    ...
    setIsLoading(true)
    try {
      if (!(window as any).Papa) throw new Error("Parsing library not loaded.")
      const parsedData = (window as any).Papa.parse(consolidatedCsvData, {
        header: true,
        dynamicTyping: true,
        skipEmptyLines: true,
      ...
      setRawData(parsedData.data as ParsedCsvRow[])
      const dataForProcessing = parsedData.data || []
      const processed = processRawData(dataForProcessing as ParsedCsvRow[])
      setProcessedData(processed)
    } catch (error) {
  ...
  const getConsumptionByZoneData = () => {
    ...
    processedData.forEach((row: WaterMeterRow) => {
      ...
      if (
        ...
        const value = row[selectedPeriod] as number || 0
  ...
  const calculateLossTrend = () => {
    ...
    const trend = availableMonths.map((month) => {
      ...
      const totalL1 = l1Meter ? l1Meter[periodKey] as number || 0 : 0
      const l2Meters = processedData.filter((row) => row.label === "L2")
      const dcMetersFromL1 = processedData.filter(
        (row) => row.label === "DC" && row.parentMeterLabel === l1Meter?.meterLabel,
      )
      const totalL2 =
        l2Meters.reduce((sum, m) => sum + (m[periodKey] as number || 0), 0) +
        dcMetersFromL1.reduce((sum, m) => sum + (m[periodKey] as number || 0), 0)
      ...
      const totalL3 =
        l3Meters.reduce((sum, m) => sum + (m[periodKey] as number || 0), 0) +
        dcMeters.reduce((sum, m) => sum + (m[periodKey] as number || 0), 0)
